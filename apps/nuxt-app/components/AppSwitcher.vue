...
const apps: Record<App, { label: string; url: string }> = {
  next: { label: 'Next.js', url: (config.public.nextUrl as string) || 'http://localhost:3000' },
  nuxt: { label: 'Nuxt', url: '/' },
  vanilla: { label: 'Vanilla TS', url: (config.public.vanillaUrl as string) || 'http://localhost:3002' },
}

const others = computed(() =>
  (Object.entries(apps) as [App, { label: string; url: string }][]).filter(
